...
print(r"domain & tr_t_size & te_t_size & tr_p_size & \\")
for domain in DOMAINS:
    ...
    for file in os.listdir(plans_dir):
        ...
        plans.append(number)
        with open(f"{plans_dir}/{file}", "r") as f:
            output = f.readlines()
            assert output[-1].startswith("; cost =")
            cost = int(output[-1].split()[3])
            tr_plan_sizes.append(cost)
